...
def _parse_json_field(value):
    """Accept a JSON-encoded string or a native list."""
...
def _save_gallery_files(files, upload_to='tours/gallery/'):
    """Save uploaded gallery files and return their URLs."""
    urls = []
    for f in files:
...
class TourSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'duration_days', 'category', 'difficulty', 'rating', 'badge',
    ...
    def to_representation(self, instance):
        ...
        model_gallery_urls = []
        for gi in instance.gallery_images.all():
            request = self.context.get('request')
            if request:
                model_gallery_urls.append(request.build_absolute_uri(gi.image.url))
            else:
    ...
    def _handle_gallery_files(self, instance):
        ...
        if gallery_files:
            new_urls = _save_gallery_files(gallery_files, 'tours/gallery/')
