"use client";

import Link from "next/link";
import Image from "next/image";
import { useEffect, useState } from "react";
import { getSiteConfig, getBlogPosts, getTours, type SiteConfig, type APITour } from "@/lib/api";
import { shouldUseUnoptimizedImage } from "@/lib/images";
import BlogImagePlaceholder from "@/components/shared/BlogImagePlaceholder";

/* ── Social media SVG icons ── */
function SocialIcon({ platform }: { platform: string }) {
  const cls = "w-5 h-5";
  switch (platform) {
    case "facebook":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      );
    case "instagram":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
        </svg>
      );
    case "twitter":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.259 5.629L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z" />
        </svg>
      );
    case "youtube":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
        </svg>
      );
    case "tiktok":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
        </svg>
      );
    case "linkedin":
      return (
        <svg className={cls} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
        </svg>
      );
    default:
      return <span className="text-xs font-bold uppercase">{platform.slice(0, 2)}</span>;
  }
}

const footerLinks = {
  company: [
    { href: "/about", label: "About Us" },
    { href: "/tours", label: "Our Tours" },
    { href: "/events", label: "Events" },
    { href: "/blog", label: "Blog" },
    { href: "/contact", label: "Contact" },
  ],
  support: [
    { href: "/faqs", label: "FAQs" },
    { href: "/booking-policy", label: "Booking Policy" },
    { href: "/terms", label: "Terms of Service" },
    { href: "/privacy", label: "Privacy Policy" },
  ],
};

export default function Footer() {
  const [cfg, setCfg] = useState<SiteConfig | null>(null);
  const [tourPackages, setTourPackages] = useState<Array<{ href: string; label: string }>>([]);
  const [latestBlogs, setLatestBlogs] = useState<
    Array<{ href: string; title: string; date: string; category: string; image: string }>
  >([]);

  useEffect(() => {
    getSiteConfig().then(setCfg).catch(() => {});
    getTours()
      .then((tours: APITour[]) => {
        const latest = [...tours]
          .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
          .slice(0, 5)
          .map((t) => ({ href: `/tours/${t.slug}`, label: t.title }));
        setTourPackages(latest);
      })
      .catch(() => {});
    getBlogPosts()
      .then((posts) =>
        setLatestBlogs(
          [...posts]
            .sort((a, b) => new Date(b.publish_date).getTime() - new Date(a.publish_date).getTime())
            .slice(0, 2)
            .map((post) => ({
              href: `/blog/${post.slug}`,
              title: post.title,
              date: post.publish_date,
              category: post.category,
              image: post.image || "",
            }))
        )
      )
      .catch(() =>
        setLatestBlogs([])
      );
  }, []);

  const socials = cfg
    ? [
        { key: "facebook", url: cfg.facebook_url },
        { key: "instagram", url: cfg.instagram_url },
        { key: "twitter", url: cfg.twitter_url },
        { key: "youtube", url: cfg.youtube_url },
        { key: "tiktok", url: cfg.tiktok_url },
        { key: "linkedin", url: cfg.linkedin_url },
      ].filter((s) => s.url)
    : [
        { key: "facebook", url: "#" },
        { key: "instagram", url: "#" },
        { key: "twitter", url: "#" },
        { key: "youtube", url: "#" },
      ];
  return (
    <footer className="bg-brand-navy text-white overflow-hidden">
      {/* ── Top CTA Banner ── */}
    

      {/* ── Main Grid ── */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">

          {/* Brand Column */}
          <div className="text-center sm:text-left">
            <Link href="/" className="inline-flex flex-col items-center sm:flex-row sm:items-center gap-2 mb-4">
              <h4 className="text-[11px] font-bold uppercase tracking-[0.28em] text-white/40">{cfg?.site_name || "Get Tours"}</h4>
              <span className="text-white/40 text-xs hidden sm:inline-block ml-3">Nepal</span>
            </Link>
            <div className="h-px w-8 bg-gradient-to-r from-brand-red to-brand-orange mb-5 mx-auto sm:mx-0" />
            <p className="text-white/50 text-sm leading-relaxed max-w-xs mx-auto sm:mx-0">
              {cfg?.site_tagline || "Nepal's premier travel platform for unforgettable local tours, cultural events, and travel inspiration."}
            </p>
            <div className="mt-6 flex flex-wrap gap-2 justify-center sm:justify-start">
              {socials.map((s) => (
                <a
                  key={s.key}
                  href={s.url || "#"}
                  target={s.url && s.url !== "#" ? "_blank" : undefined}
                  rel="noopener noreferrer"
                  className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/8 text-white/55 transition-all duration-200 hover:bg-brand-red hover:text-white hover:scale-110"
                  aria-label={s.key}
                >
                  <SocialIcon platform={s.key} />
                </a>
              ))}
            </div>
          </div>

          {/* Tour Packages */}
          <div className="hidden sm:block">
            <h4 className="text-[11px] font-bold uppercase tracking-[0.28em] text-white/40 mb-4">Tour Packages</h4>
            <div className="h-px w-8 bg-gradient-to-r from-brand-red to-brand-orange mb-5" />
            <ul className="space-y-3">
              {tourPackages.length > 0
                ? tourPackages.map((link) => (
                    <li key={link.href}>
                      <Link href={link.href} className="group flex items-center gap-2 text-sm text-white/55 transition-colors hover:text-white">
                        <span className="w-0 h-px bg-brand-red transition-all duration-300 group-hover:w-4 flex-shrink-0" />
                        <span className="line-clamp-1">{link.label}</span>
                      </Link>
                    </li>
                  ))
                : ["/tours"].map((href) => (
                    <li key={href}>
                      <Link href={href} className="group flex items-center gap-2 text-sm text-white/55 transition-colors hover:text-white">
                        <span className="w-0 h-px bg-brand-red transition-all duration-300 group-hover:w-4 flex-shrink-0" />
                        View all tours
                      </Link>
                    </li>
                  ))
              }
            </ul>
          </div>

          {/* Company */}
          <div className="hidden sm:block">
            <h4 className="text-[11px] font-bold uppercase tracking-[0.28em] text-white/40 mb-4">Company</h4>
            <div className="h-px w-8 bg-gradient-to-r from-brand-blue to-brand-green mb-5" />
            <ul className="space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="group flex items-center gap-2 text-sm text-white/55 transition-colors hover:text-white">
                    <span className="w-0 h-px bg-brand-blue transition-all duration-300 group-hover:w-4 flex-shrink-0" />
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Latest Blogs */}
          <div className="hidden sm:block">
            <h4 className="text-[11px] font-bold uppercase tracking-[0.28em] text-white/40 mb-4">Latest From Blog</h4>
            <div className="h-px w-8 bg-gradient-to-r from-brand-orange to-brand-red mb-5" />
            <div className="space-y-4">
              {latestBlogs.map((post) => (
                <Link
                  key={post.href}
                  href={post.href}
                  className="group flex gap-3 rounded-2xl bg-white/5 border border-white/8 p-3 transition-all duration-300 hover:bg-white/10 hover:border-white/16"
                >
                  <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl">
                    {post.image ? (
                      <Image
                        src={post.image}
                        alt={post.title}
                        fill
                        className="object-cover transition-transform duration-300 group-hover:scale-105"
                        sizes="56px"
                        unoptimized={shouldUseUnoptimizedImage(post.image)}
                      />
                    ) : (
                      <BlogImagePlaceholder title={post.title} compact />
                    )}
                  </div>
                  <div className="min-w-0">
                    <p className="mt-1 line-clamp-2 text-xs font-medium text-white/75 leading-relaxed">{post.title}</p>
                    <p className="mt-1 text-[10px] text-white/35">{post.date}</p>
                  </div>
                </Link>
              ))}
            </div>
          </div>
        </div>
      </div>

      {/* ── Bottom Bar ── */}
      <div className="border-t border-white/10 bg-black/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-center">
          <p className="text-white/55 text-sm font-medium">
            © All rights reserved by Golden Era Tours and Travel
          </p>
          <div className="mt-3 flex flex-wrap justify-center gap-4 sm:gap-6">
            {footerLinks.support.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-xs text-white/40 transition-colors hover:text-white"
              >
                {link.label}
              </Link>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}
