...
import { getAdminTourBookings, updateTourBookingStatus, type AdminTourBooking } from "@/lib/api";
...
export default function AdminTourBookingsPage() {
  ...
  async function handleStatusChange(id: number, currentStatus: string, newStatus: string) {
    ...
    setUpdatingId(id);
    try {
      await updateTourBookingStatus(id, newStatus, token);
      loadBookings();
    } catch (err) {
    ...
    <AdminShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-brand-navy">Tour Bookings</h2>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold text-blue-900">Latest Tour Bookings</p>
                {newCount > 0 && (
                  <span className="px-2 py-0.5 bg-blue-600 text-white text-[10px] font-bold rounded-full">{newCount} new today</span>
                )}
              </div>
            </div>
              ...
              {latest.map((b) => {
                ...
                return (
                  <div key={b.id} className={`bg-white rounded-xl p-3 border ${
                    isNew ? "border-blue-200 shadow-sm" : "border-gray-100"
                  }`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          {isNew && <span className="w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0" />}
                          <p className="text-xs font-bold text-gray-800 truncate">{b.tour_title}</p>
                        </div>
                    ...
                    <div className="flex items-center justify-between mt-2 text-[11px] text-gray-500">
                      ...
                      <span>{b.persons} person{b.persons > 1 ? "s" : ""} &middot; {timeAgo(b.created_at)}</span>
                    </div>
                  </div>
                );
              })}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <svg className="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                  ...
                  {filtered.map((b) => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-gray-50 align-top">
                        ...
                        <td className="px-4 py-3 text-gray-800">{b.tour_title}</td>
                        <td className="px-4 py-3 text-gray-600">{b.travel_date || "—"}</td>
                        <td className="px-4 py-3 text-center font-semibold">{b.persons}</td>
                  ...
                  <div key={b.id} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <h3 className="font-semibold text-gray-900 text-sm mt-0.5">{b.tour_title}</h3>
                      </div>
                      <select
                        value={b.status}
                        disabled={updatingId === b.id}
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-xs text-gray-500">
                      ...
                      <span><span className="font-semibold">Date:</span> <span className="text-gray-700">{b.travel_date || "—"}</span></span>
                      <span><span className="font-semibold">Persons:</span> <span className="text-gray-700">{b.persons}</span></span>
                      <span className="col-span-2"><span className="font-semibold">Amount:</span> <span className="text-gray-700 font-medium">USD {Number(b.total_amount).toLocaleString()}</span></span>
                      {b.special_requests && (
